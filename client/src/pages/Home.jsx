...
import LogicModel from "../components/LoginModel.jsx";
    ...
    <div className="relative min-h-screen bg-[#040404] text-white overflow-hidden">
      ...
      >
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-5">
            ...
            {userData && (
              <div
                className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5
            ...
            {!userData ? (
              <button
                className="px-4 py-2 rounded-lg border-white/20 hover:bg-white/10 text-sm"
                  ...
                  {openProfile && (
                    <>
                      ...
                      >
                        ...
                        <button className="w-full px-4 py-3 text-left text-sm
                        hover:bg-white/5" onClick={handleLogOut}>
                          Logout
                        </button>
                      </motion.div>
                    </>
      ...
      <section className="pt-44 pb-32 px-6 text-center">
        <motion.h1
          ...
          className="text-5xl md:textx-7xl font-bold tracking-tight"
        >
        ...
        >
          ...
          ,productive, production-ready website
        </motion.p>
        <button
          className="px-10 py-4 rounded-xl bg-white text-black font-semibold hover:scale-105 
             transition mt-12"
             onClick={()=>userData ? navigate("/dashboard"):
              setOpenLogin(true)
             }
        >
          {userData?"Go to Dasboard":"Get started"}
        </button>
      </section>
      ...
      {userData && websites?.length>0 && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {websites.slice(0,3).map((w,i)=>(
            ...
            key={w._id}
            whileHover={{y: -6}}
            onClick={() => navigate(`/editor${StickerIcon._id}`)}
      ...
      {openLogin && (
        <LogicModel open={openLogin} onClose={() => setOpenLogin(false)} />
      )}
    </div>
  );
}
